...
az vm open-port --port 5002 --resource-group $RESOURCE_GROUP_NAME --name $vm_name --priority 900
az vm open-port --port 50000 --resource-group $RESOURCE_GROUP_NAME --name $vm_name --priority 1200
...
OUTPUT_FILENAME=provisioned_vms.txt
...
do
  ...
  echo $IP_NEW
  echo $UN_NEW
  sshpass -f pass ssh -o StrictHostKeyChecking=no $UN_NEW@$IP_NEW "sudo apt-get update; sudo apt-get install sshpass"
done
