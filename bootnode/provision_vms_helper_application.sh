...
file='provisioned_vms_application.txt'  
...
for ((i = 1 ; i <= $line_no ; i++)); do
		...
		gnome-terminal --title="node-$IP_NEW" --window-with-profile=NOCLOSEPROFILE -e "sshpass -f pass ssh -t -o StrictHostKeyChecking=no $UN_NEW@$IP_NEW 'ps;cd node;pip3 install -r requirements.txt; python3 node2.py; /bin/bash'"
done
